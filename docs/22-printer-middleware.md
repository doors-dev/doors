...
- `gox.JobOpen` attrs can be inspected or changed until the job is serialized; they may be `nil` on container heads that emit no HTML
...
func (s *stamp) Send(j gox.Job) error {
	if open, ok := j.(*gox.JobOpen); ok && open.Tag != "d0-r" && open.Attrs != nil {
		s.count++
