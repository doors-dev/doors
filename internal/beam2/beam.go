...
type Beam[T any] interface {
	...
	sync(uint, sh.SimpleFrame) (*T, bool)
}
...
func (b *beam[T, T2]) syncEntry(seq uint, after sh.SimpleFrame) *entry[T2] {
	e, has := b.values[seq]
	if has {
		return e
	}
...
func (b *beam[T, T2]) sync(seq uint, after sh.SimpleFrame) (*T2, bool) {
	b.mu.Lock()
	defer b.mu.Unlock()
