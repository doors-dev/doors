...
func fillFragment(p *pipe) {
	...
	p.buffer.PushBack(gox.NewJobOpen(ctx, 1, gox.KindRegular, "div", gox.NewAttrs()))
	p.buffer.PushBack(gox.NewJobText(ctx, "fragment"))
	p.buffer.PushBack(gox.NewJobClose(ctx, 1, gox.KindRegular, "div"))
}
...
func (s *fragmentStamp) Send(j gox.Job) error {
	...
	if open, ok := j.(*gox.JobOpen); ok && open.Kind != gox.KindContainer && open.Attrs != nil {
