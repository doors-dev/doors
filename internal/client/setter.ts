...
class Registry {
    ...
    setAttr(id: number, name: string, value: string | null): number {
        const elements = this.registry_.get(id)
        if (!elements) {
            return 0
        }
...
export function setAttr(id: number, name: string, value: string | null): number {
    return r.setAttr(id, name, value)
}
