...
Each event method returns `ActionInto[int]`; `Into` captures how many hook requests the emitted events triggered (any failure fails the call). The completion channel delivers only after every triggered hook handler has finished — awaiting the emit means the handlers ran.
...
`$on(...)` action handlers may now return a `Promise` — the old `async actions are prohibited` error is gone.
